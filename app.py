...
app.py
------
Streamlit entry point for the AI Water Quality Prediction System.
Imports data, model, and utility modules from their respective packages.
"""
...
.hero  { background: linear-gradient(135deg, #0077b6, #00b4d8); padding: 2.5rem 2rem;
...
.card    { background: rgba(255,255,255,0.07); border-radius: 12px; padding: 1.2rem;
           text-align: center; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 0.8rem; }
...
with tab1:
    ...
    for col, (val, label) in zip([c1, c2, c3, c4], stats):
        with col:
            st.markdown(f'<div class="card"><h3>{val}</h3><p>{label}</p></div>',
                        unsafe_allow_html=True)
    ...
    for col, (icon, title, desc) in zip(cols, steps):
        with col:
            st.markdown(f'<div class="card"><h3>{icon}</h3><p><b>{title}</b><br>{desc}</p></div>',
                        unsafe_allow_html=True)
    ...
    with col1:
        ...
        bars = ax.bar(["Safe (Potable)", "Unsafe (Non-Potable)"], counts.values,
                      color=["#00b894", "#e17055"], alpha=0.85, width=0.5)
        for b in bars:
    ...
    with col2:
        ...
        ax.scatter(safe_df["pH"],   safe_df["Dissolved_Oxygen"],
                   alpha=0.35, color="#00b894", label="Safe",   s=14)
        ax.scatter(unsafe_df["pH"], unsafe_df["Dissolved_Oxygen"],
                   alpha=0.35, color="#e17055", label="Unsafe", s=14)
...
with tab2:
    ...
    col1, col2 = st.columns(2)
    with col1:
        ph       = st.slider("pH Level",               4.0,  11.0,  7.2, 0.1,
                             help="WHO standard: 6.5–8.5")
        do       = st.slider("Dissolved Oxygen (mg/L)", 0.0,  14.0,  8.0, 0.1,
                             help="Safe: >6.5 mg/L")
        turbidity = st.slider("Turbidity (NTU)",        0.0,  20.0,  2.5, 0.1,
                              help="WHO limit: <4 NTU")
        temp     = st.slider("Temperature (°C)",        5.0,  40.0, 22.0, 0.5)
    with col2:
        nitrate  = st.slider("Nitrate (mg/L)",          0.0,  50.0,  5.0, 0.5,
                             help="WHO limit: <10 mg/L")
        bod      = st.slider("BOD (mg/L)",              0.0,  15.0,  2.0, 0.1,
                             help="Good water: <3 mg/L")
        cond     = st.slider("Conductivity (µS/cm)",    100, 1500,   400,  10)
        coliform = st.slider("Coliform (CFU/100mL)",      0, 1000,    30,   5,
                             help="Safe: <50 CFU/100mL")
    ...
    if st.button("🚀 Predict Water Quality", type="primary", use_container_width=True):
        ...
        <div class="card {verdict_class}" style="padding:1.5rem; margin:1rem 0">
          ...
          <p style="font-size:1.05rem; color:#caf0f8; margin-top:0.5rem">
            ...
            Risk Level: <b style="color:{color}">{level}</b>
          </p>
...
with tab3:
    ...
    ax.set_xticklabels([feature_names[i] for i in idx], rotation=30,
...
Access to safe drinking water is a critical global challenge, particularly in wetland and
riverine ecosystems impacted by climate change and agricultural runoff. This project applies
**Machine Learning** to predict water potability from physicochemical parameters — enabling
rapid, cost-effective assessment without expensive laboratory testing.

**Problem Statement**

Traditional water quality testing is time-consuming, expensive, and inaccessible in rural
areas. An AI-driven prediction system provides instant assessments to support environmental
monitoring, aquatic ecosystem protection, and community health initiatives.
...
- **Validation:** ROC-AUC, Precision, Recall, F1-score, Confusion Matrix

**WHO Parameter Standards**

| Parameter | Safe Range | Environmental Significance |
|---|---|---|
| pH | 6.5 – 8.5 | Acidity/alkalinity balance |
| Dissolved Oxygen | > 6.5 mg/L | Aquatic biodiversity support |
| Turbidity | < 4 NTU | Sediment & pathogen indicator |
| Temperature | 15 – 27°C | Ecosystem thermal health |
| Nitrate | < 10 mg/L | Agricultural runoff indicator |
| BOD | < 3 mg/L | Organic pollution measure |
| Conductivity | 200–600 µS/cm | Dissolved mineral content |
| Coliform | < 50 CFU/100mL | Bacterial contamination |

**Real-World Applications**
- Real-time monitoring of rivers, wetlands, and lakes
- Rural drinking water safety assessment
- Climate change impact prediction on aquatic ecosystems
- Early warning systems for ecosystem degradation
- Support for fisheries and cold-chain supply monitoring
...
st.markdown("---")
st.markdown(
    '<p style="text-align:center; color:#90e0ef; font-size:0.85rem">'
    '💧 AI Water Quality Prediction | Streamlit + Scikit-learn | Sushma Shukla, VIT Vellore'
    '</p>',
    unsafe_allow_html=True,
)
