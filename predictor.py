...
predictor.py
------------
Inference utilities: single-sample prediction, WHO parameter validation,
...
class PredictionResult:
    ...
    probabilities: np.ndarray     # [P(unsafe), P(safe)]
    parameters:    List[ParameterStatus]
    recommendations: List[str]


def check_parameters(input_values: dict) -> List[ParameterStatus]:
    """
    Validate each parameter against WHO safe limits.

    Parameters
    ----------
    input_values : dict mapping feature name -> float value

    Returns
    -------
    List of ParameterStatus objects
    """
    statuses = []
    for name, (lo, hi, desc) in WHO_LIMITS.items():
        val = input_values.get(name, 0.0)
        statuses.append(ParameterStatus(name=name, value=val,
                                        lower=lo, upper=hi, description=desc))
    return statuses


def build_recommendations(statuses: List[ParameterStatus]) -> List[str]:
    """Return remediation advice for every out-of-range parameter."""
    recs = []
    for s in statuses:
        if not s.is_safe:
            recs.append(f"[{s.name}] {REMEDIATION.get(s.name, 'Investigate further.')}")
    return recs


def predict(model, scaler, input_values: dict) -> PredictionResult:
    """
    Run inference for a single water sample.

    Parameters
    ----------
    model        : trained sklearn model
    scaler       : fitted StandardScaler
    input_values : dict with keys matching WHO_LIMITS

    Returns
    -------
    PredictionResult
    """
    feature_order = list(WHO_LIMITS.keys())
    x = np.array([[input_values[f] for f in feature_order]])
...
def get_risk_level(result: PredictionResult) -> Tuple[str, str]:
    """
    Classify overall risk level based on confidence and parameter violations.

    Returns
    -------
    (level_label, hex_color)
    """
